...
const MisTickets = () => {
  ...
  useEffect(() => {
    const obtenerMisTickets = async () => {
      try {
        ...
        const respuesta = await axios.get(`https://shadow-ticket-support-backend-jace.onrender.com/api/tickets/mis-pendientes/${id_tecnico}`);
  ...
  const handleResolverTicket = async () => {
    ...
    try {
      ...
      await axios.put(`https://shadow-ticket-support-backend-jace.onrender.com/api/tickets/resolver/${ticketSeleccionado.id}`, {
        id_tecnico,
